...
function sanitizeAssessmentValues(values: unknown, allowedKeys: Set<string>) {
  if (!values || typeof values !== 'object') {
    return {} as Record<string, string>;
  }
...
async function isTeacherAllowed(examCycleId: string, classId: string, teacherId: string) {
  ...
  const academicYear = String(examCycle.academicYear || '');
  const teacherMatch = mongoose.Types.ObjectId.isValid(teacherId)
    ? [{ teacherId }, { teacherId: new mongoose.Types.ObjectId(teacherId) }]
    : [{ teacherId }];

  const [subjectClassTeacher, legacyClassTeacher, directClassTeacher] = await Promise.all([
  ...
  return Boolean(subjectClassTeacher || legacyClassTeacher || directClassTeacher);
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const allowed = await isTeacherAllowed(examCycleId, classId, sessionUser.id);
    if (!allowed) {
      return NextResponse.json(
        { success: false, error: 'Only the class teacher can manage these values' },
        { status: 403 }
      );
    }
